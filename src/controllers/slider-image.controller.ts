...
export class SliderImageController {
  ...
  async loadJson(): Promise<void> {
    return new Promise(async (resolve, reject) => {
      const content = JSON.parse(fs.readFileSync('src/assets/JSON/slide.json', {'encoding': 'utf-8'}));
  ...
  async find(
    ...
    return new Promise(async (resolve, reject) => {
      ...
      for (const imgPath of imgPaths) {
        imgFiles.push(fs.readFileSync(`src/assets/Imagens/Slide/${imgPath.path}`, {encoding: 'base64'}));
      }
